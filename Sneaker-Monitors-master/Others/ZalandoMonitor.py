...
def scrape_main_site(headers, proxy):
    ...
    """
    items = []
    #https://www.zalando.it/sneakers-uomo/?order=activation_date
    # Makes request to site
    url = 'https://www.zalando.it/sneakers-uomo/?order=activation_date'
    s = requests.Session()
    html = s.get(url=url, headers=headers, proxies=proxy, verify=False, timeout=15)
    soup = BeautifulSoup(html.text, 'html.parser')
    products = soup.find_all('div', {'class': 'kpgVnb w8MdNG cYylcv QylWsg _75qWlu iOzucJ JT3_zV DvypSJ'})
    # Stores particular details in array 
    for product in products:
        try:
            span = product.find('article')
            item = [
...
def remove_duplicates(mylist):
    ...
    """
    print ([list(t) for t in set(tuple(element) for element in mylist)])
